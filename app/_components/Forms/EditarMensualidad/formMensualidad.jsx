...
export default async function FormMensualidad(params) {

    const id = params.id
    let mensualidad = {}
    let title = "Guardar Mensualidad";
    if(params.id!=0){
        const dataMen = await fetchRequest("/mensualidad/"+id, "GET", null);

        if(dataMen.error){
            return (<><h1>{dataMen.message}</h1></>)
        }

        mensualidad = dataMen.message;
        title = "Detalles Mensualidad";
    }

    
    const dataAlum = await fetchRequest("/alumnos", "GET", null);

    const alumnos = dataAlum.message;
    const arreglo = [];

    alumnos.map(alumno=>{
        arreglo.push([alumno.id, alumno.nombres+" "+alumno.apellidos]);
    })
                        ...
                        <div className="w-full space-y-6">
                            <div className="w-full">
                                <div className="relative ">
                                    <InputSelect nombre={"alumnos"} clase={inputsClass} datos={arreglo} default={mensualidad.usuario}/>
                                </div>
                            </div>
                            <div className="w-full">
                                <div className="relative ">
                                    <InputDate nombre={"inicio"} clase={inputsClass} default={mensualidad.inicio}/>
                                </div>
                            </div>
                            <div className="w-full">
                                <div className="relative ">
                                    <InputDate nombre={"fin"} clase={inputsClass} default={mensualidad.fin}/>
                                </div>
                            </div>
                            <div className="w-full">
                                <div className="relative ">
                                    <InputText nombre={"cantidad"} clase={inputsClass} placeholder={"Cantidad"} default={mensualidad.cantidad}/>
                                </div>
                            </div>
                            <div>
